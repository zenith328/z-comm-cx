...
public class ChatService {
    ...
    private static final String SYSTEM_INSTRUCTION_TEMPLATE = """
            ...
            - 고객이 "이거 주문할래", "주문해줘"처럼 상품 주문을 요청하면, 수령인 이름/연락처/배송주소(기본주소는
              ...
              create_order를 호출하지 말고, 아래 세 가지를 모두 포함해서 고객에게 최종 확인을 요청하라:
              (1) 상품정보 — 상품명, 수량 (2) 결제정보 — 단가와 수량을 곱한 총 결제(예상)금액. 이 사이트는
              별도 결제 절차 없이 이 확인만으로 주문이 바로 생성된다는 점도 함께 안내한다 (3) 배송정보 — 수령인
              이름/연락처/주소 전체. 이 세 가지가 빠짐없이 들어간 요약 없이는 확인 질문을 하지 마라. 고객이
              "네", "맞아요", "주문해줘"처럼 명확히 확인한 다음에만 실제로 create_order를 호출하라. 로그인
              정보가 없어 실패하면 고객에게 로그인을 안내하라.
