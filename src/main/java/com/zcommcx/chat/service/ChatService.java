...
public class ChatService {
    ...
    private static final Map<String, String> TOOL_LABELS = Map.ofEntries(
            ...
            Map.entry("charge_balance", "CX-Pay 충전"),
    ...
    private static final String SYSTEM_INSTRUCTION_TEMPLATE = """
            ...
            - 고객이 "상품 보여줘", "~있어?", "무슨 브랜드 있어?" 처럼 상품을 찾거나 둘러보고 싶어하면 get_products를
              ...
              실행해도 된다.
            - 이 사이트의 유일한 결제수단은 CX-Pay다(사전에 충전해두는 잔액, 실제 카드/계좌 결제는 없음). 고객이
              "충전해줘", "만원만 충전해줘"처럼 명확히 금액을 말하면 charge_balance를 사용하라.
            - 고객이 "이거 주문할래", "주문해줘"처럼 상품 주문을 요청하면, 수령인 이름/연락처/배송주소(기본주소는
              필수, 우편번호·상세주소는 선택)를 아직 모르면 먼저 물어봐라. 만약 이전 주문 내역 등에서 이미 알고
              있는 배송정보를 그대로 재사용하려는 상황이면(고객이 새로 알려준 게 아니라면), 곧바로 최종 확인으로
              넘어가지 말고 먼저 "이전 배송정보(수령인 OOO/연락처 OOO/주소 OOO)를 그대로 사용할까요?"처럼
              배송정보 재사용 여부만 따로 확인하라. 배송정보가 확정되면(재사용 확인을 받았거나, 고객이 새로
              알려줘서 이미 확실한 경우), 그다음 단계로 아래 세 가지를 모두 포함해서 고객에게 최종 주문 확인을
              요청하라: (1) 상품정보 — 상품명, 수량 (2) 결제정보 — 단가와 수량을 곱한 총 결제금액. 이 확인
              즉시 그 금액이 고객의 CX-Pay 잔액에서 차감된다는 점도 함께 안내한다 (3) 배송정보 — 수령인
              이름/연락처/주소 전체. 이 세 가지가 빠짐없이 들어간 요약 없이는 최종 확인 질문을 하지
              마라. 고객이 "네", "맞아요", "주문해줘"처럼 명확히 확인한 다음에만 실제로 create_order를
              호출하라. 로그인 정보가 없어 실패하면 고객에게 로그인을 안내하고, CX-Pay 잔액이 부족해 실패하면
              부족한 사실을 안내하고 충전을 원하는지 물어봐라(원하면 charge_balance로 충전한 뒤 다시 주문을
              진행한다).
            - 고객이 "리뷰 어때", "후기 보여줘", "장점/단점 알려줘"처럼 전반적인 평가를 물어볼 때만
              ...
              리뷰를 먼저 나서서 붙이지 마라 — 고객이 명시적으로 요청했을 때만 호출한다.
