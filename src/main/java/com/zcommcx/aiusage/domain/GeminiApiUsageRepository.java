...
public interface GeminiApiUsageRepository extends JpaRepository<GeminiApiUsage, LocalDate> {
     ...
     * upsert로 구현했다.
     * updated_at은 SQL now()가 아니라 호출자가 넘긴 시각(Java LocalDateTime.now(), JVM
     * 시간대 설정을 따름)을 쓴다 — SQL now()는 DB 서버 자체 시간대(Supabase는 UTC)로 계산돼서
     * JVM에 -Duser.timezone=Asia/Seoul을 줘도 영향을 못 받기 때문이다.
     */
    @Modifying
    @Query(
            value = "insert into gemini_api_usage (usage_date, request_count, token_count, updated_at) "
                    ...
                    + "updated_at = :now",
            nativeQuery = true)
    void incrementUsage(@Param("date") LocalDate date, @Param("tokens") long tokens, @Param("now") LocalDateTime now);
}
